...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not data:
                raise AssertionError("Response data field is missing")
            
            if not data.get('token'):
                raise AssertionError("Token is missing in response data")
            
            if not data.get('userId'):
                raise AssertionError("UserId is missing in response data")
            
            user = data.get('user')
            if not user:
                raise AssertionError("User object is missing in response data")
            
            if user.get('role') != 0:
                raise AssertionError(f"Expected user role 0, got {user.get('role')}")

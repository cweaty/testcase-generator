...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Step 2: Make GET request to /api/books with keyword=三体
            step += 1
            ...
            if not records:
                raise Exception("data.records数组为空")
            
            # Check if any record contains "三体" in book name
            has_santi = False
            for record in records:
                book_name = record.get('name', '') or record.get('title', '') or record.get('bookName', '')
                if '三体' in book_name:
                    has_santi = True
                    break
            
            if not has_santi:
                raise Exception("data.records中没有包含'三体'的图书记录")
            
            # Take screenshot after successful verification
            await page.screenshot(path=os.path.join(SCREENSHOT_DIR, f'step_{step:02d}.png'))
    ...
    result = {
        "passed": passed,
        "message": error_msg or '搜索图书成功（按关键词）测试通过',
